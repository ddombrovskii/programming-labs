...
namespace lab2_2
{
    public partial class Form1 : Form
    {
        ...
        {
            if (Database.initialList == null || !Database.initialList.isCreated())
            {
                Database.initialList = new CycleDoubleLinkedList();
            ...
            else if (Database.initialList.isEmpty())
                MessageBox.Show("Список уже создан. Добавьте в него элементы", "Внимание", MessageBoxButtons.OK, MessageBoxIcon.Warning);
        }
        ...
        private void removeTopToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (Database.initialList != null && Database.initialList.isCreated())
            {
                ...
                MessageBox.Show("Список не существует!", "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Error);
        }
        ...
        private void processListToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (Database.initialList == null || !Database.initialList.isCreated())
                MessageBox.Show("Список не существует!", "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Error);
/*
            else if (!Database.initialList.isCreated())
                MessageBox.Show("Список не существует!", "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Error);
*/
            else if (Database.initialList.isEmpty())
                MessageBox.Show("Список пуст!", "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Error);
